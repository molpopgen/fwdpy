...
namespace fwdpy
{
  ...
  */
  ...
  inline void evolve_regions_sampler_test_details(singlepop_t * pop,
						  const unsigned long seed,
						  const unsigned * Nvector,
						  const size_t Nvector_len,
						  const double neutral,
  ...
  inline void evolve_regions_sampler_test( GSLrng_t * rng,
					   std::vector<std::shared_ptr<singlepop_t> > * pops,
					   ...
					   const char * fitness)
  {
    ...
    for(std::size_t i=0;i<pops->size();++i)
      {
	threads.emplace_back( std::thread(//std::launch::async,
					 evolve_regions_sampler_test_details,
