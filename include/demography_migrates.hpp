...
namespace fwdpy
{
  namespace demography
  {
	...
	/*!
	  ...
	  \note In Cython/Python, it must also be enforced that weights.size() == number of demes 
	  in the simulation.
	 */
      {
	...
	for( auto & i : weights)
	  {
	    if(i.size()!=s) throw std::runtime_error("all vectors of migration weights must be same length");
