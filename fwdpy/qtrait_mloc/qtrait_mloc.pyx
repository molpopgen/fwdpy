# distutils: language = c++
# distutils: sources = fwdpy/qtrait_mloc/qtrait_mloc_impl.cc

from cython.operator cimport dereference as deref,preincrement as inc
from libcpp.vector cimport vector
from libcpp.map cimport map
from libcpp.utility cimport pair
from libcpp.string cimport string
from fwdpy.fwdpy cimport *
from fwdpy.internal.internal cimport shwrappervec
from fwdpy.fwdpp cimport sep_sample_t
from fwdpy.structs cimport VAcum

import fwdpy.internal as internal
import pandas

cdef extern from "qtrait_evolve_mlocus.hpp" namespace "fwdpy::qtrait" nogil:
     void evolve_qtrait_mloc_no_sampling_async( GSLrng_t * rng,
				                vector[shared_ptr[multilocus_t]] * pops,
				                const unsigned * Nvector,
				                const size_t Nvector_length,
				                const vector[double] & neutral_mutation_rates,
				                const vector[double] & selected_mutation_rates,
				                const vector[double] & sigma_mus,
				                const vector[double] & within_region_rec_rates,
				                const vector[double] & between_region_rec_rates,
				                const double f,
				                const double sigmaE,
				                const double optimum,
				                const double VS) except +

     vector[vector[pair[uint,detailed_deme_sample]]] evolve_qtrait_mloc_sample_async( GSLrng_t * rng,
				                                                              GSLrng_t * rng_sample,
				                                                              vector[shared_ptr[multilocus_t]] * pops,
				                                                              const unsigned * Nvector,
				                                                              const size_t Nvector_length,
				                                                              const vector[double] & neutral_mutation_rates,
				                                                              const vector[double] & selected_mutation_rates,
				                                                              const vector[double] & sigma_mus,
				                                                              const vector[double] & within_region_rec_rates,
				                                                              const vector[double] & between_region_rec_rates,
				                                                              const double f,
				                                                              const double sigmaE,
				                                                              const double optimum,
				                                                              const double VS,
				                                                              const int sample,
                                                                                              const unsigned nsam) except +

     vector[vector[qtrait_stats_cython]] evolve_qtrait_mloc_popstats_async( GSLrng_t * rng,
				                                            vector[shared_ptr[multilocus_t]] * pops,
				                                            const unsigned * Nvector,
				                                            const size_t Nvector_length,
				                                            const vector[double] & neutral_mutation_rates,
				                                            const vector[double] & selected_mutation_rates,
				                                            const vector[double] & sigma_mus,
				                                            const vector[double] & within_region_rec_rates,
				                                            const vector[double] & between_region_rec_rates,
				                                            const double f,
				                                            const double sigmaE,
				                                            const double optimum,
				                                            const double VS,
				                                            const int sample) except +

     vector[vector[pair[selected_mut_data,vector[double]]]] evolve_qtrait_mloc_track_async( GSLrng_t * rng,
				                                                            vector[shared_ptr[multilocus_t]] * pops,
				                                                            const unsigned * Nvector,
				                                                            const size_t Nvector_length,
				                                                            const vector[double] & neutral_mutation_rates,
				                                                            const vector[double] & selected_mutation_rates,
				                                                            const vector[double] & sigma_mus,
				                                                            const vector[double] & within_region_rec_rates,
				                                                            const vector[double] & between_region_rec_rates,
				                                                            const double f,
				                                                            const double sigmaE,
				                                                            const double optimum,
				                                                            const double VS,
				                                                            const int sample) except +

     vector[vector[VAcum]] evolve_qtrait_mloc_VA_async( GSLrng_t * rng,
				                        vector[shared_ptr[multilocus_t]] * pops,
				                        const unsigned * Nvector,
				                        const size_t Nvector_length,
				                        const vector[double] & neutral_mutation_rates,
				                        const vector[double] & selected_mutation_rates,
				                        const vector[double] & sigma_mus,
				                        const vector[double] & within_region_rec_rates,
				                        const vector[double] & between_region_rec_rates,
				                        const double f,
				                        const double sigmaE,
				                        const double optimum,
                                                        const double VS,
				                        const int sample) except +


#POWER MEAN FUNCTIONS

cdef extern from "qtrait_evolve_mlocus.hpp" namespace "fwdpy::qtrait" nogil:
     void evolve_qtrait_mloc_pm_no_sampling_async( GSLrng_t * rng,
				                   vector[shared_ptr[multilocus_t]] * pops,
				                   const unsigned * Nvector,
				                   const size_t Nvector_length,
				                   const vector[double] & neutral_mutation_rates,
				                   const vector[double] & selected_mutation_rates,
				                   const vector[double] & sigma_mus,
				                   const vector[double] & within_region_rec_rates,
				                   const vector[double] & between_region_rec_rates,
				                   const double f,
				                   const double sigmaE,
				                   const double optimum,
				                   const double VS,
				                   const vector[double] & SLd,
					 	   const double & SLp,
			  		 	   const vector[double] & MLd,
			  		 	   const double & MLp) except +

     vector[vector[pair[uint,detailed_deme_sample]]] evolve_qtrait_mloc_pm_sample_async( GSLrng_t * rng,
				                                                         GSLrng_t * rng_sample,
				                                                         vector[shared_ptr[multilocus_t]] * pops,
				                                                         const unsigned * Nvector,
				                                                         const size_t Nvector_length,
				                                                         const vector[double] & neutral_mutation_rates,
				                                                         const vector[double] & selected_mutation_rates,
				                                                         const vector[double] & sigma_mus,
				                                                         const vector[double] & within_region_rec_rates,
				                                                         const vector[double] & between_region_rec_rates,
				                                                         const double f,
				                                                         const double sigmaE,
				                                                         const double optimum,
				                                                         const double VS,
				                                                         const vector[double] & SLd,
					 	   					 const double & SLp,
			  		 	   					 const vector[double] & MLd,
			  		 	   					 const double & MLp,
				                                                         const int sample,
                                                                                         const unsigned nsam) except +

     vector[vector[qtrait_stats_cython]] evolve_qtrait_mloc_pm_popstats_async( GSLrng_t * rng,
				                                               vector[shared_ptr[multilocus_t]] * pops,
				                                               const unsigned * Nvector,
				                                               const size_t Nvector_length,
				                                               const vector[double] & neutral_mutation_rates,
				                                               const vector[double] & selected_mutation_rates,
				                                               const vector[double] & sigma_mus,
				                                               const vector[double] & within_region_rec_rates,
				                                               const vector[double] & between_region_rec_rates,
				                                               const double f,
				                                               const double sigmaE,
				                                               const double optimum,
				                                               const double VS,
				                                               const vector[double] & SLd,
					 	   			       const double & SLp,
			  		 	   			       const vector[double] & MLd,
			  		 	   			       const double & MLp,
				                                               const int sample) except +

     vector[vector[pair[selected_mut_data,vector[double]]]] evolve_qtrait_mloc_pm_track_async( GSLrng_t * rng,
				                                                               vector[shared_ptr[multilocus_t]] * pops,
				                                                               const unsigned * Nvector,
				                                                               const size_t Nvector_length,
				                                                               const vector[double] & neutral_mutation_rates,
				                                                               const vector[double] & selected_mutation_rates,
				                                                               const vector[double] & sigma_mus,
				                                                               const vector[double] & within_region_rec_rates,
				                                                               const vector[double] & between_region_rec_rates,
				                                                               const double f,
				                                                               const double sigmaE,
				                                                               const double optimum,
				                                                               const double VS,
				                                                               const vector[double] & SLd,
					 	   					       const double & SLp,
			  		 	   					       const vector[double] & MLd,
			  		 	   					       const double & MLp,
				                                                               const int sample) except +

     vector[vector[VAcum]] evolve_qtrait_mloc_pm_VA_async( GSLrng_t * rng,
				                           vector[shared_ptr[multilocus_t]] * pops,
				                           const unsigned * Nvector,
				                           const size_t Nvector_length,
				                           const vector[double] & neutral_mutation_rates,
				                           const vector[double] & selected_mutation_rates,
				                           const vector[double] & sigma_mus,
				                           const vector[double] & within_region_rec_rates,
				                           const vector[double] & between_region_rec_rates,
				                           const double f,
				                           const double sigmaE,
				                           const double optimum,
                                                           const double VS,
                                                           const vector[double] & SLd,
					 	   	   const double & SLp,
			  		 	   	   const vector[double] & MLd,
			  		 	   	   const double & MLp,
				                           const int sample) except +




include "evolve_qtraits_mloc.pyx"
