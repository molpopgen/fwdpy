...
cdef extern from "libseq.hpp" namespace "fwdpy::libseq":
    ...
    map[string,double] libseq_extra_ld_stats( const vector[pair[double,string]] & __data, const double minfreq, const double binsize)
    map[string,double] libseq_extra_ld_stats( const vector[pair[double,string]] & __data, const double minfreq, const double binsize, const vector[double] & gmap)
